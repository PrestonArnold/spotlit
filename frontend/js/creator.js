...
const URL_FIELDS = ['urlWebsite', 'urlGitHub', 'urlLinkedIn', 'urlBluesky', 'urlMastodon', 'urlTwitter', 'urlInstagram', 'urlYouTube', 'urlHuggingFace', 'urlNuGet']
...
function persist() {
    const meta = {
        name: get('name'), role: get('role'), date: get('date'),
...
URL_FIELDS.forEach(f => { document.getElementById(f).value = meta[f] || '' })

document.querySelectorAll('.meta-card input').forEach(el => el.addEventListener('input', persist))
...
document.getElementById('modal-dismiss').addEventListener('click', () => modal.classList.remove('is-open'))
...
document.getElementById('download').addEventListener('click', () => {
    const fm = [
        ...
        'image: "./portrait.jpg"',
        'ogImage: "./og.jpg"',
        ...URL_FIELDS.map(f => get(f) ? `${f}: "${get(f)}"` : null),
        '---',
    ].filter(Boolean).join('\n')

    const body = QUESTIONS
        .map((q, i) => {
            const answer = (answers[i] ?? '').replace(/\n+$/, '').trim()
            return `## ${q}\n\n${answer}`
        })
        .join('\n\n')
    ...
    a.href = url
    a.download = `${get('name').toLowerCase().replace(/\s+/g, '-') || 'spotlit'}.md`
